-- Sprawdź obecne polityki RLS dla tabeli articles
-- I zaktualizuj polityki aby umożliwić edycję przez administratorów i edytorów
...
-- Upewnij się, że polityka SELECT działa dla zalogowanych użytkowników
CREATE POLICY IF NOT EXISTS "Authenticated users can view all articles" 
ON public.articles 
FOR SELECT 
TO authenticated
USING (true);